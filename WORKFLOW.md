...
When comparing the end-to-end development time, the second round (the precise branch) initially felt slower because it required spending several extra minutes drafting a highly detailed prompt with strict constraints. However, it proved to be significantly faster end-to-end. While the vague branch might generate code instantly, it requires extensive manual review, debugging of hidden edge cases (like timezone-related date bugs), and manual patching to reach production quality. By front-loading the requirements in the precise round, the AI generated a robust, fully-tested component on the first attempt—ultimately saving hours of debugging and review time.

## AI Mistake Caught During Development
During the implementation of the precise component, the AI made a strictness error when generating the test suite. The AI initially asserted the red error text color using `expect(element).toHaveStyle('color: red');`. However, the AI failed to account for how `@testing-library/jest-dom` strictly normalizes CSS color values during tests. When the test ran, it failed because the DOM actually returned `color: rgb(255, 0, 0)`. We caught this hallucination/oversight in the testing output and had to manually instruct the AI to rewrite the assertion to explicitly match the RGB format expected by the testing environment. This highlighted the importance of actively verifying AI-generated tests rather than assuming they will seamlessly integrate with strict third-party libraries.
